...
Later on during the boot process, SW may also choose to block read access to the SW managed partitions via the corresponding `*_READ_LOCK` registers, e.g. [`CREATOR_SW_CFG_READ_LOCK`](registers.md#creator_sw_cfg_read_lock).
