...
module prim_lfsr #(
  ...
  input                         clk_i,
  input                         rst_ni,
  input                         load_ext_en_i, // loads externally seed into the state (takes precedence)
  input        [LfsrDw-1:0]     seed_ext_i,    // external seed input
  ...
  assign lfsr_d = (load_ext_en_i)       ? seed_ext_i      :
                  (lfsr_en_i && lockup) ? Seed            :
  ...
  `ASSERT(LfsrLockupCheck_A, lfsr_en_i && lockup && !load_ext_en_i |=> !lockup, clk_i, !rst_ni)
  ...
  `ASSERT(NextStateCheck_A, lfsr_en_i && !load_ext_en_i |=> lfsr_q ==
    compute_next_state(coeffs, $past(entropy_i,1), $past(lfsr_q,1)),
    clk_i, !rst_ni )

  // check that external seed is correctly loaded into the state
  `ASSERT(ExtSeedInputCheck_A, load_ext_en_i |=> lfsr_q == $past(seed_ext_i), clk_i, !rst_ni)
...
`endif
  ...
  if (MaxLenSVALocal) begin : gen_max_len_sva
    ...
    assign perturbed_d = perturbed_q | (|entropy_i) | load_ext_en_i;
    ...
    `ASSERT(MaximalLengthCheck0_A, cnt_q == 0 |-> lfsr_q == Seed, clk_i, !rst_ni || perturbed_q)
    `ASSERT(MaximalLengthCheck1_A, cnt_q != 0 |-> lfsr_q != Seed, clk_i, !rst_ni || perturbed_q)
  end
